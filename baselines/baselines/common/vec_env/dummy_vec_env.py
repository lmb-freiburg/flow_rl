...
class ImVecEnv(VecEnv):
    def __init__(self, env_fns):
        ...
        for key, value in self.observation_space.items():
            self.buf_obs[key] = np.zeros((self.num_envs,) + tuple(value.shape), value.dtype)
        self.buf_dones = np.zeros((self.num_envs,), dtype=np.bool)
        self.buf_rews  = np.zeros((self.num_envs,), dtype=np.float32)
        self.buf_infos = [{} for _ in range(self.num_envs)]
        self.actions = None
    ...
    def step_wait(self):
        for i in range(self.num_envs):
            obs_dict, self.buf_rews[i], self.buf_dones[i], self.buf_infos[i] = self.envs[i].step(self.actions[i])
            if self.buf_dones[i]:
                obs_dict = self.envs[i].reset()
            for key, value in obs_dict.items():
                self.buf_obs[key][i] = value
        return (self._obs_from_buf(), np.copy(self.buf_rews), np.copy(self.buf_dones),
                self.buf_infos.copy())
    ...
    def _obs_from_buf(self):
        out_dict = {}
        for key, value in self.buf_obs.items():
            out_dict[key] = np.copy(value)
        return out_dict
